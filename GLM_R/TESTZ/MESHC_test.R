...
mesh = fdaPDE::create.mesh.2D(nodes=nodes, segments = boundary_segments)
# x11()
...
if(location_flag){
  ...
  xobs=runif(min=-1,max=4,n=80)
  yobs=runif(min=-1,max=1,n=80)
  loc=cbind(xobs,yobs)
}else{
  loc = mesh$nodes
}
...
Whilelm.fit = gam.fem( data = observed.data ,desmat=desmat ,fdobj = simfd ,log.lambda.seq = lambda, max.steps=15, 
                       fam=FAMILY, mesh.const=T, method.phi=1, psi=NULL, show=F,
