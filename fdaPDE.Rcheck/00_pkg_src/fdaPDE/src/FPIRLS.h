...
VectorXr root_W_; // (G^-2 * Var_function(mu)^-1)^1/2 it is a vector for the same reason as above
...
// Bernoulli
template <typename InputHandler, typename Integrator, UInt ORDER, UInt mydim, UInt ndim>
class FPIRLS_Bernoulli : public FPIRLS_Base {
...
inline void initialize_mu(const VectorXr& y) {
    for(UInt i = 0; i < y.size(); i++ )
        mu_[i] = 1/2 * (y[i] + 1/2); // It is different for binary or non-binary outcomes
};


// Poisson
template <typename InputHandler, typename Integrator, UInt ORDER, UInt mydim, UInt ndim>
class FPIRLS_Poisson : public FPIRLS_Base {
...
inline void initialize_mu(const VectorXr & y) { mu_ = y;} // It is different for binary or non-binary outcomes
