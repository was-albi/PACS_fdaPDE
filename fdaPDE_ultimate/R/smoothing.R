...
getBetaCoefficients<-function(locations, observations, fit.FEM, covariates, incidence_matrix = NULL, ndim, mydim, weights_matrix = NULL)
{
  loc_nodes = NULL
  fnhat = NULL
  betahat = NULL
  ...
  if(!is.null(weights_matrix)){
    chol_w = chol(weights_matrix)
    observations = chol_w%*%observations
    covariates = chol_w%*%covariates
  }
...
getBetaCoefficientsFromCPP<-function(locations=NULL, observations, FEMbasis, ndim, mydim, covariates, weights_matrix = NULL, fit.FEM, dataHandler=c("RegressionData","RegressionDataElliptic","RegressionDataEllipticSpaceVarying"))
{
  loc_nodes = NULL
  fnhat = NULL
  betahat = NULL
